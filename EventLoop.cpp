...
// 创建wakeipfd，用来notify唤醒subReactor处理新来的channel
int createEventfd()
{
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        // Poller监听两类fd     一种是client的fd，一种是wakeupfd(用于mainloop和subloop之间的通信)
        ...
        /**
         * IO线程 mainLoop accept   [fd <= channel(打包fd)]   subloop
...
void EventLoop::runInLoop(Functor cb)
{
    ...
    else    //在非当前loop线程中执行cb，就需要唤醒对应的loop所在线程，执行cb
    {
        queueInLoop(cb);
    }
}
